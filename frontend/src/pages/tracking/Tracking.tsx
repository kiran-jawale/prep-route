...
export default function Tracking() {
    ...
    return (
      <div className="p-8">
        <div className="py-16 text-center">
          <h3 className="text-xl font-medium">No Test Found</h3>
          <p className="mt-2 text-zinc-500">Unable to load test.</p>
        </div>
      </div>
    );
  }
  ...
  const steps = [
    ...
    {
      title: "Live",
      completed: status === "live" || status === "expired",
      current: status === "live",
    },
    ...
    <div className="space-y-8 p-8">
      <div className="rounded-2xl border bg-white p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-2 text-zinc-500">Current Test Lifecycle</p>
          </div>
          ...
          {steps.map((step, index) => (
            <li key={step.title} className="relative flex-1">
              {index < steps.length - 1 && (
                <div
                  className={`absolute left-1/2 top-4 h-1 w-full ${
                    step.completed ? "bg-[#6475F7]" : "bg-zinc-200"
                  }`}
                />
              )}
              ...
              <div className="relative flex flex-col items-center">
                ...
                >
                  {step.completed ? <Check size={18} /> : <Circle size={12} />}
                </span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-2xl border bg-white p-5">
          <p className="text-sm text-zinc-500">Scheduled At</p>

          <p className="mt-2 font-medium">
            {formatDate((test as any)?.scheduledAt)}
          </p>
        </div>
        ...
        <div className="rounded-2xl border bg-white p-5">
          ...
          <p className="mt-2 font-medium">
            {formatDate((test as any)?.publishedAt)}
          </p>
        </div>

        <div className="rounded-2xl border bg-white p-5">
          <p className="text-sm text-zinc-500">Expires At</p>

          <p className="mt-2 font-medium">
            {formatDate((test as any)?.availableUntil)}
          </p>
        </div>
